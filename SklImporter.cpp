...
void SklImporter::readFile(const wchar_t* path)
{
	...
	if (fileVersion == 1953262451)
	{
		...
		for (int i = 0; i < numBones; ++i)
		{
			...
			inFile.read(reinterpret_cast<char*>(&bones.at(i).scaleX), 4);
			bones.at(i).scaleX *= 10.0f;
			bones.at(i).scaleY = bones.at(i).scaleZ = bones.at(i).scaleX;
	...
	else if (fileVersion == 587026371)
	{
		...
		for (int i = 0; i < numBones; ++i)
		{
			...
			inFile.read(reinterpret_cast<char*>(&bones.at(i).scaleX), 4);
			inFile.read(reinterpret_cast<char*>(&bones.at(i).scaleY), 4);
			inFile.read(reinterpret_cast<char*>(&bones.at(i).scaleZ), 4);
