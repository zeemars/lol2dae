...
void ColladaWriter::createNodeStringStream()
{
	...
	for (auto& i : nodes)
	{
		...
		else
		{
			...
			rotationMatrix(0, 0) = bones.at(i).localMatrix(0, 0);
			rotationMatrix(0, 1) = bones.at(i).localMatrix(0, 1);
			rotationMatrix(0, 2) = bones.at(i).localMatrix(0, 2);
			rotationMatrix(1, 0) = bones.at(i).localMatrix(1, 0);
			rotationMatrix(1, 1) = bones.at(i).localMatrix(1, 1);
			rotationMatrix(1, 2) = bones.at(i).localMatrix(1, 2);
			rotationMatrix(2, 0) = bones.at(i).localMatrix(2, 0);
			rotationMatrix(2, 1) = bones.at(i).localMatrix(2, 1);
			rotationMatrix(2, 2) = bones.at(i).localMatrix(2, 2);

			rotations = rotationMatrix.eulerAngles(0, 1, 2);
			rotations[0] *= MathsUtility::degree;
			rotations[1] *= MathsUtility::degree;
			rotations[2] *= MathsUtility::degree;
			...
			nodeStream << "\t\t\t\t" << R"(<rotate sid ="rotateX">1 0 0 )" << rotations[0] << "</rotate>" << endl;
			nodeStream << "\t\t\t\t" << R"(<rotate sid="rotateY">0 1 0 )" << rotations[1] << "</rotate>" << endl;
			nodeStream << "\t\t\t\t" << R"(<rotate sid ="rotateZ">0 0 1 )" << rotations[2] << "</rotate>" << endl;
		}
	}
...
void ColladaWriter::writeHeader()
{
	...
	outFile << "\t\t\t" << "<authoring_tool>lol2dae v1.0</authoring_tool>" << endl;
	outFile << "\t\t" << "</contributor>" << endl;
...
void ColladaWriter::writeSkin()
{
	...
	for (auto& i : boneIndices)
	{
		...
		Matrix<float, 4, 4, RowMajor> inverseMatrix;
		inverseMatrix = bones.at(i).globalMatrix.inverse();

		for (int j = 0; j < 16; ++j)
		{
			inverseMatrixString << inverseMatrix(j) << " ";
		}
	}
	...
	for (auto& i : vertices)
	{
		vertexCountString << i.numWeights << " ";

		for (int j = 0; j < i.numWeights; ++j)
		{
			int weightIndex;

			if (i.boneWeight[j] >= 1.0f)
			{
				weightIndex = 0;
			}
			...
			jointIndiceString << static_cast<int>(i.boneId[j]) << " " << weightIndex << " ";
		}
	}
...
void ColladaWriter::writeAnimation()
{
	...
	ostringstream interpolationStream;
	float cumulativeFrameDelay = 0;

	for (int i = 0; i < animation.numFrames; ++i)
	{
		inputStream << cumulativeFrameDelay << " ";
		cumulativeFrameDelay += animation.frameDelay;
		interpolationStream << "STEP ";
	}
	...
	for (auto& i : animation.bones)
	{
		...
		outFile << "\t\t\t\t\t" << R"(<float_array id=")" << i.name << R"(TranslationInputArray" count=")" << animation.numFrames << R"(">)" << endl;
		outFile << "\t\t\t\t\t" << inputStream.str().c_str() << endl;
		outFile << "\t\t\t\t\t" << "</float_array>" << endl;
		outFile << "\t\t\t\t\t" << "<technique_common>" << endl;
		outFile << "\t\t\t\t\t" << R"(<accessor source="#)" << i.name << R"(TranslationInputArray" count=")" << animation.numFrames << R"(">)" << endl;
		...
		outFile << "\t\t\t\t\t" << R"(<Name_array id=")" << i.name << R"(TranslationInterpolationArray" count=")" << animation.numFrames << R"(">)" << endl;
		outFile << "\t\t\t\t\t" << interpolationStream.str().c_str() << endl;
		outFile << "\t\t\t\t\t" << "</Name_array>" << endl;
		outFile << "\t\t\t\t\t" << "<technique_common>" << endl;
		outFile << "\t\t\t\t\t" << R"(<accessor source="#)" << i.name << R"(TranslationInterpolationArray" count=")" << animation.numFrames << R"(">)" << endl;
		...
		outFile << "\t\t\t\t\t" << R"(<float_array id=")" << i.name << R"(TranslationArray" count=")" << animation.numFrames * 3 << R"(">)" << endl;
		...
		outFile << "\t\t\t\t\t" << "</float_array>" << endl;
		outFile << "\t\t\t\t\t" << "<technique_common>" << endl;
		outFile << "\t\t\t\t\t" << R"(<accessor count=")" << animation.numFrames << R"(" stride="3" source="#)" << i.name << R"(TranslationArray">)" << endl;
		outFile << "\t\t\t\t\t" << R"(<param name="X" type="float"/>)" << endl;
		outFile << "\t\t\t\t\t" << R"(<param name="Y" type="float"/>)" << endl;
		outFile << "\t\t\t\t\t" << R"(<param name="Z" type="float"/>)" << endl;
		outFile << "\t\t\t\t\t" << "</accessor>" << endl;
		...
		outFile << "\t\t\t\t\t" << R"(<float_array id=")" << i.name << R"(RotateXInputArray" count=")" << animation.numFrames << R"(">)" << endl;
		outFile << "\t\t\t\t\t" << inputStream.str().c_str() << endl;
		outFile << "\t\t\t\t\t" << "</float_array>" << endl;
		outFile << "\t\t\t\t\t" << "<technique_common>" << endl;
		outFile << "\t\t\t\t\t" << R"(<accessor source="#)" << i.name << R"(RotateXInputArray" count=")" << animation.numFrames << R"(">)" << endl;
		outFile << "\t\t\t\t\t" << R"(<param name="TIME" type="float"/>)" << endl;
		outFile << "\t\t\t\t\t" << "</accessor>" << endl;
		outFile << "\t\t\t\t\t" << "</technique_common>" << endl;
		outFile << "\t\t\t\t" << "</source>" << endl;
		...
		outFile << "\t\t\t\t\t" << R"(<Name_array id=")" << i.name << R"(RotateXInterpolationArray" count=")" << animation.numFrames << R"(">)" << endl;
		outFile << "\t\t\t\t\t" << interpolationStream.str().c_str() << endl;
		outFile << "\t\t\t\t\t" << "</Name_array>" << endl;
		outFile << "\t\t\t\t\t" << "<technique_common>" << endl;
		outFile << "\t\t\t\t\t" << R"(<accessor source="#)" << i.name << R"(RotateXInterpolationArray" count=")" << animation.numFrames << R"(">)" << endl;
		outFile << "\t\t\t\t\t" << R"(<param name="INTERPOLATION" type="Name"/>)" << endl;
		outFile << "\t\t\t\t\t" << "</accessor>" << endl;
		outFile << "\t\t\t\t\t" << "</technique_common>" << endl;
		outFile << "\t\t\t\t" << "</source>" << endl;
		...
		vector<Vector3f> ea;
		ea.resize(animation.numFrames);

		for (size_t a = 0; a < ea.size(); ++a)
		{
			Matrix<float, 3, 3, RowMajor> matrix;
			matrix = i.quaternion.at(a).matrix();
			ea.at(a) = matrix.eulerAngles(0, 1, 2);

			ea.at(a)[0] *= MathsUtility::degree;
			ea.at(a)[1] *= MathsUtility::degree;
			ea.at(a)[2] *= MathsUtility::degree;
		}

		outFile << "\t\t\t\t" << R"(<source id=")" << i.name << R"(RotateX">)" << endl;
		outFile << "\t\t\t\t\t" << R"(<float_array id=")" << i.name << R"(RotateXArray" count=")" << animation.numFrames << R"(">)" << endl;
		...
		outFile << "\t\t\t\t\t" << "</float_array>" << endl;
		outFile << "\t\t\t\t\t" << "<technique_common>" << endl;
		outFile << "\t\t\t\t\t" << R"(<accessor count=")" << animation.numFrames << R"(" source="#)" << i.name << R"(RotateXArray">)" << endl;
		...
		outFile << "\t\t\t\t\t" << R"(<float_array id=")" << i.name << R"(RotateYInputArray" count=")" << animation.numFrames << R"(">)" << endl;
		outFile << "\t\t\t\t\t" << inputStream.str().c_str() << endl;
		outFile << "\t\t\t\t\t" << "</float_array>" << endl;
		outFile << "\t\t\t\t\t" << "<technique_common>" << endl;
		outFile << "\t\t\t\t\t" << R"(<accessor source="#)" << i.name << R"(RotateYInputArray" count=")" << animation.numFrames << R"(">)" << endl;
		outFile << "\t\t\t\t\t" << R"(<param name="TIME" type="float"/>)" << endl;
		outFile << "\t\t\t\t\t" << "</accessor>" << endl;
		outFile << "\t\t\t\t\t" << "</technique_common>" << endl;
		outFile << "\t\t\t\t" << "</source>" << endl;
		...
		outFile << "\t\t\t\t\t" << R"(<Name_array id=")" << i.name << R"(RotateYInterpolationArray" count=")" << animation.numFrames << R"(">)" << endl;
		outFile << "\t\t\t\t\t" << interpolationStream.str().c_str() << endl;
		outFile << "\t\t\t\t\t" << "</Name_array>" << endl;
		outFile << "\t\t\t\t\t" << "<technique_common>" << endl;
		outFile << "\t\t\t\t\t" << R"(<accessor source="#)" << i.name << R"(RotateYInterpolationArray" count=")" << animation.numFrames << R"(">)" << endl;
		outFile << "\t\t\t\t\t" << R"(<param name="INTERPOLATION" type="Name"/>)" << endl;
		outFile << "\t\t\t\t\t" << "</accessor>" << endl;
		outFile << "\t\t\t\t\t" << "</technique_common>" << endl;
		outFile << "\t\t\t\t" << "</source>" << endl;
		...
		outFile << "\t\t\t\t\t" << R"(<float_array id=")" << i.name << R"(RotateYArray" count=")" << animation.numFrames << R"(">)" << endl;
		...
		outFile << "\t\t\t\t\t" << "</float_array>" << endl;
		outFile << "\t\t\t\t\t" << "<technique_common>" << endl;
		outFile << "\t\t\t\t\t" << R"(<accessor count=")" << animation.numFrames << R"(" source="#)" << i.name << R"(RotateYArray">)" << endl;
		outFile << "\t\t\t\t\t" << R"(<param name="ANGLE" type="float"/>)" << endl;
		outFile << "\t\t\t\t\t" << "</accessor>" << endl;
		outFile << "\t\t\t\t\t" << "</technique_common>" << endl;
		outFile << "\t\t\t\t" << "</source>" << endl;
		...
		outFile << "\t\t\t\t\t" << R"(<float_array id=")" << i.name << R"(RotateZInputArray" count=")" << animation.numFrames << R"(">)" << endl;
		outFile << "\t\t\t\t\t" << inputStream.str().c_str() << endl;
		outFile << "\t\t\t\t\t" << "</float_array>" << endl;
		outFile << "\t\t\t\t\t" << "<technique_common>" << endl;
		outFile << "\t\t\t\t\t" << R"(<accessor source="#)" << i.name << R"(RotateZInputArray" count=")" << animation.numFrames << R"(">)" << endl;
		outFile << "\t\t\t\t\t" << R"(<param name="TIME" type="float"/>)" << endl;
		outFile << "\t\t\t\t\t" << "</accessor>" << endl;
		outFile << "\t\t\t\t\t" << "</technique_common>" << endl;
		outFile << "\t\t\t\t" << "</source>" << endl;
		...
		outFile << "\t\t\t\t\t" << R"(<Name_array id=")" << i.name << R"(RotateZInterpolationArray" count=")" << animation.numFrames << R"(">)" << endl;
		outFile << "\t\t\t\t\t" << interpolationStream.str().c_str() << endl;
		outFile << "\t\t\t\t\t" << "</Name_array>" << endl;
		outFile << "\t\t\t\t\t" << "<technique_common>" << endl;
		outFile << "\t\t\t\t\t" << R"(<accessor source="#)" << i.name << R"(RotateZInterpolationArray" count=")" << animation.numFrames << R"(">)" << endl;
		outFile << "\t\t\t\t\t" << R"(<param name="INTERPOLATION" type="Name"/>)" << endl;
		outFile << "\t\t\t\t\t" << "</accessor>" << endl;
		outFile << "\t\t\t\t\t" << "</technique_common>" << endl;
		outFile << "\t\t\t\t" << "</source>" << endl;
		...
		outFile << "\t\t\t\t\t" << R"(<float_array id=")" << i.name << R"(RotateZArray" count=")" << animation.numFrames << R"(">)" << endl;
		...
		outFile << "\t\t\t\t\t" << "</float_array>" << endl;
		outFile << "\t\t\t\t\t" << "<technique_common>" << endl;
		outFile << "\t\t\t\t\t" << R"(<accessor count=")" << animation.numFrames << R"(" source="#)" << i.name << R"(RotateZArray">)" << endl;
		outFile << "\t\t\t\t\t" << R"(<param name="ANGLE" type="float"/>)" << endl;
		outFile << "\t\t\t\t\t" << "</accessor>" << endl;
		outFile << "\t\t\t\t\t" << "</technique_common>" << endl;
		outFile << "\t\t\t\t" << "</source>" << endl;
