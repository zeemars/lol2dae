...
void ColladaWriter::createNodeStringStream()
{
	...
	for (auto& i : nodes)
	{
		...
		else
		{
			...
			nodeStream << "\t\t\t\t" << R"(<scale sid="scale">)" << bones.at(i).scaleX << " " << bones.at(i).scaleY << " " << bones.at(i).scaleZ << R"(</scale>)" << endl;
		}
	}
...
void ColladaWriter::writeHeader()
{
	...
	outFile << "\t\t\t" << "<authoring_tool>lol2dae v1.2</authoring_tool>" << endl;
	outFile << "\t\t" << "</contributor>" << endl;
...
void ColladaWriter::writeSkin()
{
	...
	for (auto& i : vertices)
	{
		...
		for (int j = 0; j < 4; ++j)
		{
			...
			if (i.boneWeight[j] <= 0.0f)
			{
				continue;
			}

			else if (i.boneWeight[j] >= 1.0f)
			{
				weightIndex = 0;
			}
...
void ColladaWriter::writeAnimation()
{
	...
	for (auto& i : animation.bones)
	{
		...
		for (auto& k : i.translation)
		{
			...
			translationInterpolationStream << "STEP ";
		}
		...
		{
			outFile << t.second[0] << " " << t.second[1] << " " << t.second[2] << " ";
		}
		...
		for (auto& k : i.quaternion)
		{
			...
			rotationInterpolationStream << "STEP ";
		}
		...
		for (size_t a = 0; a < ea.size(); ++a)
		{
			...
			matrix = i.quaternion.at(a).second.matrix();
		...
		for (auto& s : i.scale)
		{
			outFile << s.second[0] << " " << s.second[1] << " " << s.second[2] << " ";
		}
